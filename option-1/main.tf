...
resource "azurerm_public_ip" "example" {
  name                = "pubip-${var.env_prefix}-${var.env_name}"
  location            = azurerm_resource_group.example.location
  resource_group_name = azurerm_resource_group.example.name
...
resource "azurerm_network_interface" "example" {
  name                = "nic-${var.env_prefix}-${var.env_name}"
  location            = azurerm_resource_group.example.location
  resource_group_name = azurerm_resource_group.example.name
  tags                = local.all_tags
  ...
  ip_configuration {
    name                          = "ipconf-${var.env_prefix}-${var.env_name}"
    subnet_id                     = azurerm_subnet.example.id
...
resource "azurerm_linux_virtual_machine" "example" {
  name                  = "vm-${var.env_prefix}-${var.env_name}"
  location              = azurerm_resource_group.example.location
  resource_group_name   = azurerm_resource_group.example.name
  ...
  os_disk {
    name                 = "os-disk-${var.env_prefix}-${var.env_name}"
    caching              = "ReadWrite"
  ...
  computer_name  = "${var.env_prefix}-${var.env_name}"
...
resource "azurerm_virtual_machine_extension" "example" {
  name                  = "cs-nginx-${var.env_prefix}-${var.env_name}"
